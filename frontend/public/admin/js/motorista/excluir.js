async function carregarDetalhesMotorista() {
  try {
    ...
    const response = await axios.get(
      `http://localhost:3000/admin/motoristas/exibir/${id}`
    );
    ...
    const cpfElem = document.createElement("div");
    cpfElem.innerHTML = `<div><strong>CPF:</strong> ${motorista.cpf}</div>`;
    detalhesMotorista.append(cpfElem);

    const nascimentoElem = document.createElement("div");
    nascimentoElem.innerHTML = `<div><strong>Data de Nascimento:</strong> ${motorista.nascimento}</div>`;
    detalhesMotorista.append(nascimentoElem);
...
async function deletar(event) {
  ...
  if (confirmacao) {
    try {
      const id = window.location.pathname.split("/").pop();
      await axios.delete(`http://localhost:3000/admin/motorista/deletar/${id}`);
...
document
  .querySelector("#deletar-motorista")
  .addEventListener("submit", deletar);
